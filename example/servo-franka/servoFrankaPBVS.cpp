...
int main(int argc, char **argv)
{
  ...
  std::string opt_robot_ip = "192.168.1.102";
  ...
  try {
    std::cout << "Attempting to connect with robot at " << opt_robot_ip << " ..." << "\n";
    robot.connect(opt_robot_ip);

    std::cout << "Robot connected." << "\n\n";

    std::cout << "Connecting to RealSense camera..." << "\n";
    vpRealSense2 rs;
    rs2::config config;
    ...
    std::cout << "Accessing camera intrinsic and extrinsic parameters..." << "\n";
    ...
    if (!opt_eMc_filename.empty()) {
      std::cout << "Loading extrinsics from file..." << "\n";
    ...
    // Servo
    vpHomogeneousMatrix cdMc, cMo_t, cMo_e, cdMo, oMo;
    vpHomogeneousMatrix cMo_t_lock, cMo_e_lock;
    ...
    if (opt_plot) {
      plotter = new vpPlot(2, 250 * 2, 500, I.getWidth() + 80, 10, "Aescape - Calibration Test");
      plotter->setTitle(0, "Visual features error");
      plotter->setTitle(1, "EE velocities");
    ...
    static double t_reset_plot = vpTime::measureTimeMs();
    ...
    std::vector<vpImagePoint> effector_points;
    std::vector<vpImagePoint> target_points;

    while (!has_converged && !final_quit) {
      if (!inControlLoop) {
        inControlLoop = true;
        std::cout << "Executing control loop..." << std::endl;
      }
      ...
      for (size_t i = 0; i < cMo_vec.size(); i++) {
        vpHomogeneousMatrix cMo_i = cMo_vec[i];
        vpDisplay::displayFrame(I, cMo_i, cam, opt_tagSize / 2, vpColor::none, 3);
      ...
      if (target_idx != -1 && effector_idx != -1 && set_target) {
        set_target = false;
        has_target = true;
      } 
      ...
      if (has_target) {
        for (size_t j = 0; j < target_points.size(); j++)
          vpDisplay::displayCross(I, target_points[j], 14, vpColor::red, 3);
        for (size_t j = 0; j < effector_points.size(); j++)
          vpDisplay::displayCross(I, effector_points[j], 14, vpColor::red, 3);
        vpDisplay::displayFrame(I, cMo_t_lock, cam, opt_tagSize / 2, vpColor::none, 3);

      }

      // effector and target tags are detected
      if ((target_idx != -1 && effector_idx != -1 )  || has_target) { // (cMo_vec.size() == 1) {

        cMo_t = has_target ? cMo_t_lock : cMo_vec[target_idx];
        cMo_e = has_target ? cMo_e_lock : cMo_vec[effector_idx];

        static bool first_time = true;
        if (first_time) {
          std::cout << "Detected a both tags..." << std::endl;
          // Introduce security wrt tag positionning in order to avoid PI rotation
          std::vector<vpHomogeneousMatrix> v_oMo(2), v_cdMc(2);
          v_oMo[1].buildFrom(0, 0, 0, 0, 0, M_PI);
          for (size_t i = 0; i < 2; i++) {
            // v_cdMc[i] = cdMo * v_oMo[i] * cMo_t.inverse();
            v_cdMc[i] = cMo_e * v_oMo[i] * cMo_t.inverse();
          }
        ...
        // VVS
        //cdMc = cdMo * oMo * cMo_t.inverse();
        cdMc = cMo_e * oMo * cMo_t.inverse();
        t.buildFrom(cdMc);
        tu.buildFrom(cdMc);
        ...
        if (opt_task_sequencing) {
          ...
          v_c = task.computeControlLaw((vpTime::measureTimeMs() - t_init_servo)/10000.);
        }
        else {
